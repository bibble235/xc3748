...
// Create a 2 byte optional data structure
#[derive(Debug)]
...
impl Xc3748 {
    pub fn new(port: Box<dyn serialport::SerialPort>, feedback: Feedback) -> Xc3748 {
        Xc3748 {
            port,
            buffer: [0; 8],
            feedback,
    ...
    fn send_command(&mut self, command: u8, optional_data: OptionalData) {
        ...
        self.port.write(&self.buffer).expect("Write failed");
    }
